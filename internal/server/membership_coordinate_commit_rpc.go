...
// CoordinateJoinCommit pauses old members, completes final catch-up,
// and durably commits the exact candidate on every old member.
func (s *GRPCServer) CoordinateJoinCommit(
	ctx context.Context,
	...
	if err := s.commitOldMembersForJoin(ctx, candidate); err != nil {
		return nil, err
	}
